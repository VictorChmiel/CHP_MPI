...
int main(int argc, char ** argv)
{
	...
	for (i=0; i<..; i++) // y=ny
	  grille[...] = valeurs_aux_bords[2];

	for (i=0; i<...; i++) // x=0
	  grille[...] = valeurs_aux_bords[3];
	...
	for (j=1; j<...; j++)
	{
	  for (i=...; i<nx-1; i++) 
	  {
	    for (int n=0; n<...; n++) // on effectue nb_tirages tirages de particules
	    {
		  ...
		  if ( ( position[0] == 0 ) || ( position[0] == ... ) || ( position[1] == ... ) || ( position[1] == ny-1 ) )
		  {
		    valeur = grille[position[1]*... + position[0]]; // On récupère la valeur au bord atteint
		    stop = ...;
		  }
	      }
